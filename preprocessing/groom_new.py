...
def extract_gz_files(root_dir):
    gz_files = glob(os.path.join(root_dir, '**/Predictions.nii.gz'), recursive=True)
    for gz_file in gz_files:
        print(gz_file)
        extract_dir = os.path.splitext(gz_file)[0]  # Remove .gz extension
...
def start_grooming(args):
    ...
    search_pattern = os.path.join(args.data_dir, "**", "*Predictions.nii")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='C:\\Users\\Jakar\\Downloads\\Hippocampus_Study\\disease_progression\\Hippo_Predictions\\subjects\\')
