...
def run(model, train_loader, test_loader, epochs, optimizer, scheduler, writer,
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save(model.state_dict(), "/home/jakaria/Explaining_Shape_Variability/src/DeepLearning/compute_canada/guided_vae/data/CoMA/raw/hippocampus/models_attribute_age_limited_tle/model_state_dict.pt")
        torch.save(model_c.state_dict(), "/home/jakaria/Explaining_Shape_Variability/src/DeepLearning/compute_canada/guided_vae/data/CoMA/raw/hippocampus/models_attribute_age_limited_tle/model_c_state_dict.pt")
...
def train(model, optimizer, model_c, optimizer_c, model_c_2, optimizer_c_2, loader, device, beta, w_cls, guided, guided_contrastive_loss, correlation_loss, attribute_loss, temp, delta, threshold):
    ...
    for data in loader:
        ...
        if guided_contrastive_loss:
            ...
            loss_snn_reg = SNN_Loss_Reg(z, label[:, :, 0])
        ...
        if attribute_loss:
            ...
            #print(label[:, :, 0].shape)
            #cls
            loss_attr_cls = loss_attr(z[:,0], label[:, :, 1])
            loss += loss_attr_cls * w_cls
            #reg
            loss_attr_reg = loss_attr(z[:,1], label[:, :, 0])
